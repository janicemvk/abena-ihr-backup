# ABENA Password Migration Script - Phase 2
# Step 6: Migrate passwords from plain text to bcrypt

Write-Host "=====================================" -ForegroundColor Cyan
Write-Host "ABENA Password Migration - Phase 2" -ForegroundColor Cyan
Write-Host "=====================================" -ForegroundColor Cyan
Write-Host ""
...
$latestBackup = Get-ChildItem ".\backups\backup_pre_security_*.sql" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1

if (-not $latestBackup) {
    Write-Host "[ERROR] No backup found!" -ForegroundColor Red
    Write-Host "Please run .\integrate-security-clean.ps1 first to create a backup." -ForegroundColor Red
    exit 1
}

Write-Host "[OK] Backup found: $($latestBackup.Name)" -ForegroundColor Green
...
try {
    ...
    Write-Host ""
    Write-Host "[ERROR] Dry-run failed. Error: $_" -ForegroundColor Red
...
Write-Host "[OK] Additional backup created: $preMigrationBackup" -ForegroundColor Green
Write-Host ""
...
try {
    ...
    Write-Host ""
    Write-Host "[OK] Password migration completed successfully!" -ForegroundColor Green
    Write-Host ""
    ...
    if ($verification -gt 0) {
        Write-Host "[OK] Verified: $verification passwords successfully hashed with bcrypt" -ForegroundColor Green
    } else {
        Write-Host "[WARN] Warning: No bcrypt passwords found. Migration may have failed." -ForegroundColor Yellow
    }
    
} catch {
    Write-Host ""
    Write-Host "[ERROR] Migration failed. Error: $_" -ForegroundColor Red
    Write-Host ""
...
Write-Host "=====================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "[OK] All passwords migrated to bcrypt" -ForegroundColor Green
...
Write-Host ""
Write-Host "To continue, run: .\update-services-security-clean.ps1" -ForegroundColor Cyan
